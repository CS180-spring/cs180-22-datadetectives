...
class CarAvgMapReduce : public IMapReduce {
 public:
  ...
  std::pair<std::string, int> Map(const std::string& record) override {
    ...
    std::string key = fields[3];  // Modify this line to average over different keys. Using car make
    int price = std::stoi(fields[6]);  // Using sales price
